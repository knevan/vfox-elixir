...
function elixir_utils.peek_lua_table(o, indent)
    ...
    local function handle_table(t, currentIndent)
        ...
        for k, v in pairs(t) do
            local key = type(k) ~= 'number' and ('[' .. elixir_utils.peek_lua_table(k, currentIndent + 1) .. ']') or ''
            local value = type(v) == 'table' and handle_table(v, currentIndent + 1) or
                elixir_utils.peek_lua_table(v, currentIndent + 1)
...
function elixir_utils.check_platform()
    if RUNTIME.OS_TYPE == "windows" then
        error(
            "Windows is not supported. Please direct use the offcial installer to setup Elixir. visit: https://elixir-lang.org/install.html#windows")
    end
end
...
function elixir_utils.windows_install_exe(version)
    ...
    if string.match(version, "^main%-latest") then
    ...
    local install_cmd = installer ..
        " -Wait -PassThru" ..
        " /S /D=" .. RUNTIME.pluginDirPath .. "\\..\\..\\cache\\elixir\\v-" .. version .. "\\elixir-" .. version
...
function elixir_utils.check_version_existence(url)
    ...
    if err ~= nil or resp.status_code ~= 200 then
        error(
            "Please confirm whether the corresponding Elixir release version exists! visit: https://github.com/elixir-lang/elixir/releases")
    end
end
...
function elixir_utils.get_elixir_release_verions_in_linux()
    local resp, err = http.get({
        url = "https://fastly.jsdelivr.net/gh/knevan/vfox-elixir@main/assets/versions.txt"
    })
    local result = {}
...
function elixir_utils.get_elixir_release_verions_in_windows()
    local resp, err = http.get({
        url = "https://fastly.jsdelivr.net/gh/knevan/vfox-elixir@main/assets/versions_win.txt"
    })
    local result = {}
    for version in string.gmatch(resp.body, '([^\n]+)') do
        table.insert(result, {
